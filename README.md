![Welcome GIF](images/welcome.gif)
Привет! Я **Вита**, и это моё персональное портфолио, где я показываю свои работы и проекты.

---

## 🎨 Обо мне
Я создаю яркие и эстетичные презентации, помогаю учить английский легко и интересно.

---
## 🛠️ Технологии
- HTML5 & CSS3  
- Анимации через CSS (`@keyframes`)  
- Flexbox для адаптивной верстки  
- Встроенные видео и аудио  
- Canva для проектов
- English B2

---

## 📬 Контакты
Связаться со мной можно так:

- ✉ Email: [vita.nikulina@icloud.com](mailto:vita.nikulina@icloud.com)  
- 📞 Телефон: +375 25 524 6017  
- 🌐 Социальные сети:  
  [VK](https://vk.com/) | [Instagram](https://instagram.com/) | [Telegram](https://t.me/)
